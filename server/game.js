...
class Game4P {
  ...
  respondBet(playerIndex, action) {
    ...
    let nextLevel = null; if (action === 'retruco' && level === 'truco') nextLevel = 'retruco'; if (action === 'valenove' && level === 'retruco') nextLevel = 'valenove'; if (action === 'valedoze' && level === 'valenove') nextLevel = 'valedoze';
    if (nextLevel) { this.betState.level = nextLevel; this.betState.responderTeam = challenger % 2; this.betState.challenger = playerIndex; this.emit('betCalled', { challenger: playerIndex, level: nextLevel, responderTeam: challenger % 2 }, 'all'); for (let i = 0; i < NUM_PLAYERS; i++) if (i % 2 === challenger % 2 && !this.players[i].isBot) this.emit('turnToRespond', { responderTeam: challenger % 2 }, this.players[i].id); this.scheduleOfflineResponse(); return true; }
    return false;
  }
  scheduleOfflineTurn() {
    if (this.turnStage !== 'play') return; const playerIndex = this.currentPlayer; const player = this.players[playerIndex]; if (!player || player.isBot || player.online !== false) return;
    if (this.offlineActionTimer) clearTimeout(this.offlineActionTimer);
    this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'play' || this.currentPlayer !== playerIndex) return; const hand = this.hands[playerIndex]; if (!hand || hand.length === 0) return; const card = chooseCard(hand, this.vira.rank); this.playCard(playerIndex, card); }, 1000 + Math.random() * 1200);
  }
  scheduleOfflineResponse() {
    if (this.turnStage !== 'respond' || !this.betState) return;
    const responderTeam = this.betState.responderTeam;
    const candidates = [responderTeam, responderTeam + 2].filter(index => this.players[index] && !this.players[index].isBot);
    if (candidates.length === 0) { const botIndex = [responderTeam, responderTeam + 2].find(index => this.players[index]?.isBot); if (botIndex !== undefined) this.respondBet(botIndex, chooseBotBet(this.hands[botIndex], this.vira.rank, this.betState.level)); return; }
    if (this.offlineActionTimer) clearTimeout(this.offlineActionTimer);
    const decisionPlayer = candidates.find(index => this.players[index].online === false) ?? candidates[0];
    this.offlineActionTimer = setTimeout(() => {
      this.offlineActionTimer = null;
      if (this.turnStage !== 'respond' || !this.betState) return;
      const hand = this.hands[decisionPlayer];
      if (!hand) return;
      const action = chooseBotBet(hand, this.vira.rank, this.betState.level);
      this.respondBet(decisionPlayer, action);
    }, DECISION_TIMEOUT);
  }
