...
class Game4P {
  ...
  playCard(playerIndex, card) {
    if (!isValidPlayerIndex(playerIndex) || !isValidCard(card) || this.turnStage !== 'play' || playerIndex !== this.currentPlayer) {
      return false;
    }
  ...
  respondMaoDe11(playerIndex, action) {
    if (
      ...
      !isValidMaoDe11Action(action) ||
      this.turnStage !== 'mao11Decision' ||
      !this.maoDe11 ||
      this.maoDe11DecisionMade ||
      playerIndex % 2 !== this.maoDe11Team
    ) return false;
    ...
    if (action === 'flee') {
      ...
      this.emit(
        'handEnd',
        { winnerTeam: winningTeam, points: 1, scores: this.scores, setWins: this.setWins, draw: false, maoDe11Flee: true },
        'all'
      );
  ...
  respondBet(playerIndex, action) {
    ...
    for (let i = 0; i < NUM_PLAYERS; i++) {
      if (i % 2 === this.betState.responderTeam && !this.players[i].isBot) {
        this.emit('turnToRespond', { responderTeam: this.betState.responderTeam }, this.players[i].id);
      }
    }
