...
class Game4P {
  ...
  startNewHand() {
    ...
    this.deck = buildDeck(); shuffle(this.deck);
    ...
    if (this.scores[0] === MAO_DE_11_TRIGGER && this.scores[1] === MAO_DE_11_TRIGGER) { this.handValue = 1; this.maoDe11 = false; this.maoDe11Team = null; this.maoDeFerro = true; }
    else if (this.scores[0] === MAO_DE_11_TRIGGER || this.scores[1] === MAO_DE_11_TRIGGER) { this.handValue = 3; this.maoDe11 = true; this.maoDe11Team = this.scores[0] === MAO_DE_11_TRIGGER ? 0 : 1; this.maoDeFerro = false; }
    else { this.handValue = 1; this.maoDe11 = false; this.maoDe11Team = null; this.maoDeFerro = false; }
    this.maoDe11DecisionMade = false; this.currentPlayer = (this.dealerIndex + 3) % NUM_PLAYERS; this.turnStage = this.maoDe11 ? 'mao11Decision' : 'play'; this.betState = null; this.lastBetTeam = null;
    this.roundWins = [0, 0]; this.roundWinners = []; this.currentRound = 0; this.roundCards = []; this.playersInRound = 0; this.roundStarter = this.currentPlayer;
    for (let i = 0; i < NUM_PLAYERS; i++) if (!this.players[i].isBot) {
  ...
  playCard(playerIndex, card) {
    if (!isValidPlayerIndex(playerIndex) || !isValidCard(card) || this.turnStage !== 'play' || playerIndex !== this.currentPlayer) return false;
    const hand = this.hands[playerIndex]; const cardIndex = hand.findIndex(c => c.suit === card.suit && c.rank === card.rank); if (cardIndex === -1) return false;
    const played = hand.splice(cardIndex, 1)[0]; if (this.offlineActionTimer) { clearTimeout(this.offlineActionTimer); this.offlineActionTimer = null; }
    ...
    this.roundCards[this.currentRound][playerIndex] = played; this.playersInRound++; this.emit('cardPlayed', { player: playerIndex, card: played }, 'all');
    if (this.playersInRound === NUM_PLAYERS) this.resolveRound(); else { this.currentPlayer = (playerIndex + 3) % NUM_PLAYERS; this.emit('turn', { currentPlayer: this.currentPlayer }, 'all'); this.scheduleOfflineTurn(); }
    return true;
  }
  resolveRound() {
  ...
  advanceToNextHand() { if (this.offlineActionTimer) { clearTimeout(this.offlineActionTimer); this.offlineActionTimer = null; } this.dealerIndex = (this.dealerIndex + 1) % NUM_PLAYERS; if (this.onBeforeNewHand) this.onBeforeNewHand(); const checkBot = this.checkBotTurn; setTimeout(() => { this.startNewHand(); if (checkBot) checkBot(); }, 1500); }
  endHand(winningTeam) { const points = winningTeam === -1 ? 0 : this.handValue; if (winningTeam !== -1) this.scores[winningTeam] += points; this.emit('handEnd', { winnerTeam: winningTeam, points, scores: this.scores, setWins: this.setWins, draw: winningTeam === -1 }, 'all'); if (this.checkSetOver(winningTeam)) return; this.advanceToNextHand(); }
  respondMaoDe11(playerIndex, action) {
    ...
    this.turnStage = 'play'; this.handValue = 3; this.emit('maoDe11Started', { team: this.maoDe11Team, handValue: 3, currentPlayer: this.currentPlayer }, 'all'); this.emit('turn', { currentPlayer: this.currentPlayer }, 'all'); this.scheduleOfflineTurn(); return true;
  }
  scheduleMaoDe11Decision() {
    ...
    if (teamPlayers.every(index => this.players[index] && this.players[index].isBot)) { this.respondMaoDe11(teamPlayers[0], 'play'); return; }
    ...
    this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'mao11Decision' || !this.maoDe11 || this.maoDe11DecisionMade) return; this.respondMaoDe11(decisionPlayer, 'play'); }, DECISION_TIMEOUT);
  }
  fleeHand(playerIndex) {
    ...
    const fleeingTeam = playerIndex % 2; const winningTeam = 1 - fleeingTeam; this.scores[winningTeam] += this.handValue; this.emit('handEnd', { winnerTeam: winningTeam, points: this.handValue, scores: this.scores, setWins: this.setWins, draw: false }, 'all'); if (this.checkSetOver(winningTeam)) return true; this.advanceToNextHand(); return true;
  }
  callBet(playerIndex, betType) {
    ...
    if (betType === 'truco' && this.handValue >= 3) return false; if (betType === 'retruco' && this.handValue >= 6) return false; if (betType === 'valenove' && this.handValue >= 9) return false; if (betType === 'valedoze' && this.handValue >= 12) return false;
    const challengerTeam = playerIndex % 2; if (this.handValue >= 3 && this.lastBetTeam === challengerTeam) return false; const responderTeam = 1 - challengerTeam;
    this.betState = { challenger: playerIndex, level: betType, responderTeam, responded: false }; this.turnStage = 'respond'; this.emit('betCalled', { challenger: playerIndex, level: betType, responderTeam }, 'all');
  ...
  respondBet(playerIndex, action) {
    ...
    const responderTeam = this.betState.responderTeam; if (playerIndex % 2 !== responderTeam) return false; const { challenger, level } = this.betState;
    if (this.offlineActionTimer) { clearTimeout(this.offlineActionTimer); this.offlineActionTimer = null; }
    ...
    this.handValue = BET_VALUES[nextLevel]; this.betState = { challenger: playerIndex, level: nextLevel, responderTeam: 1 - responderTeam, responded: false }; this.lastBetTeam = playerIndex % 2; this.emit('betRaised', { challenger: playerIndex, level: nextLevel, responderTeam: 1 - responderTeam }, 'all');
  ...
  scheduleOfflineResponse() {
    ...
    if (this.offlineActionTimer) clearTimeout(this.offlineActionTimer);
    const team = this.betState.responderTeam; const teamPlayers = [team, team + 2];
    const humanPlayers = teamPlayers.filter(index => this.players[index] && !this.players[index].isBot);
    if (humanPlayers.length === 0) { const botIndex = teamPlayers.find(index => this.players[index] && this.players[index].isBot); if (botIndex !== undefined) { this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'respond' || !this.betState) return; const p = this.players[botIndex]; const action = chooseBotBet(this.hands[botIndex], this.vira.rank, this.betState.level, { hand: this.hands[botIndex], vira: this.vira, handValue: this.handValue, maoDe11: this.maoDe11, betState: this.betState }); this.respondBet(botIndex, action); }, 700); } return; }
    ...
    this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'respond' || !this.betState) return; const p = this.players[responsePlayer]; if (!p || p.isBot) return; const action = chooseBotBet(this.hands[responsePlayer], this.vira.rank, this.betState.level, { hand: this.hands[responsePlayer], vira: this.vira, handValue: this.handValue, maoDe11: this.maoDe11, betState: this.betState }); this.respondBet(responsePlayer, action); }, DECISION_TIMEOUT);
  }
  scheduleOfflineTurn() {
    ...
    if (player.isBot) { this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'play' || this.currentPlayer !== this.players.indexOf(player)) return; const context = { hand: this.hands[this.currentPlayer], vira: this.vira, handValue: this.handValue, maoDe11: this.maoDe11, betState: this.betState, players: this.players }; const action = chooseCard(context.hand, context.vira.rank, context); this.playCard(this.currentPlayer, action); }, 700); return; }
    if (player.online === false) { const playerIndex = this.currentPlayer; this.offlineActionTimer = setTimeout(() => { this.offlineActionTimer = null; if (this.turnStage !== 'play' || this.currentPlayer !== playerIndex) return; const context = { hand: this.hands[playerIndex], vira: this.vira, handValue: this.handValue, maoDe11: this.maoDe11, betState: this.betState, players: this.players }; const action = chooseCard(context.hand, context.vira.rank, context); this.playCard(playerIndex, action); }, DECISION_TIMEOUT); }
  }
}
