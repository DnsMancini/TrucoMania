...
function isValidCard(card) {
  return Boolean(
    ...
    typeof card.suit === 'string' && SUITS.includes(card.suit) &&
    typeof card.rank === 'string' && RANKS.includes(card.rank)
  );
}
...
class Game4P {
  ...
  startNewHand() {
    ...
    if (this.scores[0] === MAO_DE_11_TRIGGER && this.scores[1] === MAO_DE_11_TRIGGER) {
      ...
      this.handValue = 3; this.maoDe11 = true;
    ...
    this.betState = null; this.lastBetTeam = null; this.roundWins = [0, 0]; this.roundWinners = [];
    this.currentRound = 0; this.roundCards = []; this.playersInRound = 0; this.roundStarter = this.currentPlayer;
    for (let i = 0; i < NUM_PLAYERS; i++) {
      if (!this.players[i].isBot) {
        this.emit('handStart', {
          player: i, hand: this.hands[i], vira: this.vira, currentPlayer: this.currentPlayer, dealer: this.dealerIndex,
          handValue: this.handValue, scores: this.scores, setWins: this.setWins, maoDe11: this.maoDe11,
          maoDe11Team: this.maoDe11Team, maoDe11DecisionMade: this.maoDe11DecisionMade, maoDeFerro: this.maoDeFerro,
          turnStage: this.turnStage, players: this.players.map(p => ({ name: p.name, isBot: p.isBot, online: p.online }))
        }, this.players[i].id);
  ...
  resolveRound() {
    ...
    if (this.currentRound >= 2) {
      const first = this.roundWinners[0]; const winningTeam = winnerTeam !== -1 ? winnerTeam : first;
  ...
  respondMaoDe11(playerIndex, action) {
    ...
    if (this.turnStage !== 'mao11Decision' || !this.maoDe11 || this.maoDe11DecisionMade) return false;
    ...
    this.turnStage = 'play'; this.handValue = 3;
  ...
  fleeHand(playerIndex) {
    ...
    if (this.turnStage !== 'play' || this.maoDe11 || this.maoDeFerro || playerIndex !== this.currentPlayer) return false;
    const fleeingTeam = playerIndex % 2; const winningTeam = 1 - fleeingTeam;
  ...
  callBet(playerIndex, betType) {
    ...
    if (this.maoDe11 || this.maoDeFerro || this.turnStage !== 'play' || playerIndex !== this.currentPlayer || this.betState) return false;
  ...
  respondBet(playerIndex, action) {
    ...
    if (this.offlineActionTimer) { clearTimeout(this.offlineActionTimer); this.offlineActionTimer = null; }
    if (action === 'flee') {
      const points = this.getBetValueBefore(level); const challengerTeam = challenger % 2;
    ...
    if (action === 'accept') {
      this.handValue = BET_VALUES[level]; this.lastBetTeam = challenger % 2; this.betState = null; this.turnStage = 'play';
    ...
    if (nextLevel) {
      this.betState.level = nextLevel; this.betState.responderTeam = challenger % 2; this.betState.challenger = playerIndex;
      this.emit('betCalled', { challenger: playerIndex, level: nextLevel, responderTeam: challenger % 2 }, 'all');
      for (let i = 0; i < NUM_PLAYERS; i++) if (i % 2 === challenger % 2 && !this.players[i].isBot) this.emit('turnToRespond', { responderTeam: challenger % 2 }, this.players[i].id);
      this.scheduleOfflineResponse();
      return true;
    }
    return false;
  ...
  scheduleOfflineTurn() {
    ...
    const playerIndex = this.currentPlayer; const player = this.players[playerIndex];
    if (!player || player.isBot || player.online !== false) return;
    if (this.offlineActionTimer) clearTimeout(this.offlineActionTimer);
    this.offlineActionTimer = setTimeout(() => {
      this.offlineActionTimer = null;
      if (this.turnStage !== 'play' || this.currentPlayer !== playerIndex) return;
      const hand = this.hands[playerIndex]; if (!hand || hand.length === 0) return;
      const card = chooseCard(hand, this.vira.rank); this.playCard(playerIndex, card);
  ...
  scheduleOfflineResponse() {
    ...
    const responderTeam = this.betState.responderTeam;
    const playerIndex = [responderTeam, responderTeam + 2].find(index => this.players[index] && !this.players[index].isBot && this.players[index].online === false);
    if (playerIndex === undefined) return;
    if (this.offlineActionTimer) clearTimeout(this.offlineActionTimer);
    this.offlineActionTimer = setTimeout(() => {
      this.offlineActionTimer = null;
      if (this.turnStage !== 'respond' || !this.betState) return;
      const hand = this.hands[playerIndex]; if (!hand) return;
      const action = chooseBotBet(hand, this.vira.rank, this.betState.level); this.respondBet(playerIndex, action);
