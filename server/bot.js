...
function getStyle(context = {}) {
  return context.style || { aggression: 1, bluff: 0.11, courage: 1, patience: 1 };
}
...
function chooseCard(hand, viraRank, context = {}) {
  ...
  const strongestOnTable = currentRoundCards.reduce((max, card) => !card ? max : Math.max(max, cardStrength(card, viraRank)), -Infinity);
  ...
  if (strongestOnTable === -Infinity) {
    if (currentRound === 0) {
      if (info.top >= 14 && info.second >= 10 && Math.random() < 0.20) return validCards.find(item => item.strength === info.second)?.card || sortedAsc[0].card;
      const r = Math.random();
