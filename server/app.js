...
const io = new Server(server, { cors: { origin: CLIENT_URL === '*' ? true : CLIENT_URL, methods: ['GET', 'POST'] } });
...
(() => {
  ...
  const hideReconnectButton = () => { if (reconnectButton) reconnectButton.style.display = 'none'; };
    ...
    if (!reconnectButton) {
      ...
      reconnectButton.onclick = () => {
        ...
        socket.emit('reconnectToGame', (result) => {
          if (result?.ok) { hideReconnectButton(); return; }
  ...
  const setupReconnectGameState = () => {
    ...
    socket.on('authenticated', (data) => data?.reconnectAvailable ? showReconnectButton() : hideReconnectButton());
    socket.on('gameStateRestore', hideReconnectButton);
  };
  ...
  const setupLobbyRoomControls = () => {
    ...
    if (!entryPanel || !lobbyMenu || !createButton) return;

    // Os controles já existem no index.html. O código anterior saía daqui
    // quando encontrava #roomVisibility e, por isso, o game.js original
    // enviava createRoom antes de autenticar o Socket.IO.
    if (document.getElementById('roomVisibility')) {
      createButton.onclick = async () => {
        const user = typeof auth !== 'undefined' ? auth.currentUser : null;
        if (!user) return alert('Não autenticado. Faça login novamente.');
        const authenticated = await authenticateSocket(user);
        if (!authenticated) return alert('Não autenticado. Tente novamente.');
        ...
        socket.emit('createRoom', { playerName: name, options: { visibility, fillWithBots } }, (result) => {
    ...
    controls.innerHTML = \`
      <label style="display:flex;align-items:center;gap:8px;font-size:13px;"><span>Tipo</span><select id="roomVisibility" class="lobby-input" style="flex:1;min-width:0;"><option value="public">🌐 Pública</option><option value="private">🔒 Privada</option></select></label>
      <label style="display:flex;align-items:center;gap:8px;font-size:13px;"><input id="roomFillBots" type="checkbox" checked /><span>Completar com bots</span></label>
    \`;
    ...
    joinByCodeContainer.innerHTML = \`<input id="roomCodeInput" class="lobby-input" placeholder="Código da sala" maxlength="4" autocomplete="off" style="text-transform:uppercase;flex:1;" /><button id="joinCodeBtn" class="lobby-button">Entrar por código</button>\`;
    ...
    roomCodeInput.addEventListener('keydown', (event) => { if (event.key === 'Enter') joinByCode(); });
    roomCodeInput.addEventListener('input', () => { roomCodeInput.value = roomCodeInput.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(0, 4); });
    ...
    randomMatchButton.onclick = () => {
      ...
      socket.emit('randomMatch', name, (result) => {
        if (result?.error) { randomMatchButton.disabled = false; randomMatchButton.textContent = '🎲 Partida Aleatória'; return alert(result.error); }
        if (result?.createNew) {
          socket.emit('createRoom', name, { visibility: 'public', fillWithBots: true }, (createResult) => {
            if (createResult?.error) { randomMatchButton.disabled = false; randomMatchButton.textContent = '🎲 Partida Aleatória'; return alert(createResult.error); }
  ...
  const setup = () => {
    ...
    socket.emit = (...args) => {
      ...
      authenticateSocket(user).then((authenticated) => {
        if (!authenticated) { if (callback) callback({ error: 'Não autenticado' }); return; }
        if (callback) args.push(callback);
        originalEmit(...args);
      });
      return socket;
    };
    socket.on('authenticated', (data) => { socketAuthenticated = true; if (data?.reconnectAvailable) showReconnectButton(); else hideReconnectButton(); });
    socket.on('authError', () => { socketAuthenticated = false; });
    socket.on('disconnect', () => { socketAuthenticated = false; });
    socket.on('connect', () => { socketAuthenticated = false; if (auth.currentUser) authenticateSocket(auth.currentUser); });
    auth.onAuthStateChanged((user) => {
      if (user) authenticateSocket(user);
      else { socketAuthenticated = false; hideReconnectButton(); if (socket.connected) socket.disconnect(); }
    });
...
app.use(express.static(path.join(__dirname, '..', 'client'), {
  setHeaders: (res, filePath) => {
    if (/\.(?:js|css|png|jpg|jpeg|webp|gif|svg|mp3|wav|ico)$/i.test(filePath)) res.setHeader('Cache-Control', 'public, max-age=86400, stale-while-revalidate=604800');
  }
}));

handleSocket(io);
const PORT = process.env.PORT || 3000;
server.listen(PORT, () => { console.log(`TrucoMania rodando na porta ${PORT}`); });
