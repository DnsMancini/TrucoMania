...
function rotateArrayForPlayer(arr, startIndex) { return arr.map((_, i) => arr[(startIndex - i + arr.length) % arr.length]); }

function createCardHTML(card) {
  const suitSym = suitSymbol(card.suit);
  const corClasse = (card.suit === 'copas' || card.suit === 'ouros') ? 'naipe-vermelho' : 'naipe-preto';
  return `<div class="carta-corner top-left ${corClasse}">${card.rank}${suitSym}</div><div class="carta-center ${corClasse}">${card.rank}${suitSym}</div><div class="carta-corner bottom-right ${corClasse}">${card.rank}${suitSym}</div>`;
}
...
if (randomMatchBtn) randomMatchBtn.onclick = () => {
  ...
  socket.emit('randomMatch', { playerName: name }, (res) => {
    if (res?.error) return alert(res.error);
    if (res?.createNew) {
      socket.emit('createRoom', { playerName: name, options: { visibility: 'public', fillWithBots: true } }, (createRes) => {
...
socket.on('roomsUpdate', (rooms) => {
  ...
  if (!rooms.length) { roomsListEl.innerHTML = '<div class="lobby-empty">Nenhuma sala disponível no momento</div>'; return; }
  rooms.forEach(room => {
    const div = document.createElement('div'); div.className = 'room-item';
    div.innerHTML = `<div class="room-item-main"><div class="room-tier">Mesa competitiva</div><strong class="room-code">Sala ${room.code}</strong><div class="room-meta">${room.players}/4 jogadores <span class="room-live-dot"></span> Ao vivo</div></div><button class="join-room-btn">Entrar</button>`;
    div.querySelector('.join-room-btn').addEventListener('click', () => joinRoomFromList(room.code)); roomsListEl.appendChild(div);
  });
});

socket.on('lobbyCountdown', ({ count }) => { if (!contagemNumero || gameActive) return; contagemNumero.textContent = String(Math.max(0, count)); if (count <= 0) contagemEl.classList.add('oculto'); });

function resetOpponentHands(handsRemaining = [3, 3, 3, 3]) {
  for (let visual = 1; visual <= 3; visual++) {
    const el = HAND_SLOTS[visual]; if (!el) continue; el.innerHTML = '';
    ...
    for (let i = 0; i < remaining; i++) { const carta = document.createElement('div'); carta.className = 'carta virada'; el.appendChild(carta); }
  }
}
...
let canonicalGameStateRestoreRegistered = false; const originalSocketOn = socket.on.bind(socket); socket.on = function (eventName, ...args) { if (eventName === 'gameStateRestore' && canonicalGameStateRestoreRegistered) return this; return originalSocketOn(eventName, ...args); };
socket.on('gameStateRestore', (data) => {
  if (!data || myPlayerIndex === null || data.player !== myPlayerIndex) myPlayerIndex = data?.player ?? myPlayerIndex; if (myPlayerIndex === null || !Array.isArray(data?.players)) return;
  renderGeneration++; gameActive = true; currentGameCode = data.roomCode || currentGameCode; isMaoDeFerro = Boolean(data.maoDeFerro); playerHand = isMaoDeFerro ? Array.from({ length: Number.isInteger(data.handsRemaining?.[myPlayerIndex]) ? data.handsRemaining[myPlayerIndex] : 3 }, () => ({ hidden: true })) : (Array.isArray(data.hand) ? data.hand.slice() : []);
...
socket.on('roundResult', ({ round, winner }) => { infoRodadaEl.textContent = round >= 2 ? 'Mão encerrada' : `Rodada ${round + 2} de 3`; const bolinhas = painelHistorico.querySelectorAll('.bolinha-rodada'); if (bolinhas[round]) { let corClasse = 'bolinha-ouro'; if (winner !== -1) corClasse = winner % 2 === myPlayerIndex % 2 ? 'bolinha-verde' : 'bolinha-azul'; bolinhas[round].className = 'bolinha-rodada ' + corClasse; } const generationAtSchedule = renderGeneration; setTimeout(() => { if (generationAtSchedule !== renderGeneration || renderedRound !== round) return; mesaCartas.innerHTML = ''; }, 1200); });
socket.on('handEnd', ({ winnerTeam, points, scores }) => { gameActive = false; isMaoDe11Hand = false; isMaoDe11Decision = false; isMaoDeFerro = false; aguardandoResposta = false; isRespondingToBet = false; currentBetLevel = null; lastBetTeam = null; isMyTurn = false; clearTurnTimer(); teamAScoreEl.textContent = scores[0]; teamBScoreEl.textContent = scores[1]; if (points === 6) audioSeis?.play().catch(() => {}); else if (points === 9) audioNove?.play().catch(() => {}); else if (points === 12) audioDoze?.play().catch(() => {}); btnTruco.classList.add('oculto'); btnCorrer.classList.add('oculto'); maoDiv.innerHTML = ''; hand1.innerHTML = ''; hand2.innerHTML = ''; hand3.innerHTML = ''; mesaCartas.innerHTML = ''; viraEl.classList.add('oculto'); if (winnerTeam !== -1) mostrarMensagem(winnerTeam === myPlayerIndex % 2 ? 'Seu time ganhou a mão!' : 'Time adversário ganhou a mão.'); else mostrarMensagem('Mão empatada — ninguém pontua.'); esconderSeta(); atualizarInfoLive(); });
...
socket.on('betCalled', (data) => { handleBetChallenge(data); const audioByLevel = { truco: audioTruco, retruco: audioSeis, valenove: audioNove, valedoze: audioDoze }; (audioByLevel[data.level] || audioTruco)?.play().catch(() => {}); });
...
btnTruco.onclick = () => { if (!gameActive) return; if (isMaoDe11Decision) { socket.emit('respondMaoDe11', 'play'); clearTurnTimer(); return; } if (isMaoDe11Hand) return; if (isRespondingToBet) { const canRaise = currentBetLevel === 'truco' || currentBetLevel === 'retruco' || currentBetLevel === 'valenove'; if (!canRaise) socket.emit('respondBet', 'accept'); else { const aumentar = !window.confirm('OK = Aceitar\nCancelar = Aumentar aposta'); let raiseTo = 'retruco'; if (currentBetLevel === 'retruco') raiseTo = 'valenove'; else if (currentBetLevel === 'valenove') raiseTo = 'valedoze'; socket.emit('respondBet', aumentar ? raiseTo : 'accept'); } aguardandoResposta = false; isRespondingToBet = false; clearTurnTimer(); return; } if (!isMyTurn || aguardandoResposta || isMaoDeFerro) return; let betType = 'truco'; if (currentHandValue >= 9) betType = 'valedoze'; else if (currentHandValue >= 6) betType = 'valenove'; else if (currentHandValue >= 3) betType = 'retruco'; socket.emit('callBet', betType); clearTurnTimer(); };
...
function clearTurnTimer() { if (turnTimerInterval) { clearInterval(turnTimerInterval); turnTimerInterval = null; } cronometroEl.classList.add('oculto'); }
