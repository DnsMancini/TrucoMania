...
socket.on('handEnd', ({ winnerTeam, points, scores }) => { gameActive = false; isMaoDe11Hand = false; isMaoDe11Decision = false; isMaoDeFerro = false; aguardandoResposta = false; isRespondingToBet = false; currentBetLevel = null; lastBetTeam = null; isMyTurn = false; clearTurnTimer(); teamAScoreEl.textContent = scores[0]; teamBScoreEl.textContent = scores[1]; if (points === 6) audioSeis?.play().catch(() => {}); else if (points === 9) audioNove?.play().catch(() => {}); else if (points === 12) audioDoze?.play().catch(() => {}); btnTruco.classList.add('oculto'); btnCorrer.classList.add('oculto'); maoDiv.innerHTML = ''; hand1.innerHTML = ''; hand2.innerHTML = ''; hand3.innerHTML = ''; viraEl.classList.add('oculto'); if (winnerTeam !== -1) mostrarMensagem(winnerTeam === myPlayerIndex % 2 ? 'Seu time ganhou a mão!' : 'Time adversário ganhou a mão.'); else mostrarMensagem('Mão empatada — ninguém pontua.'); esconderSeta(); atualizarInfoLive(); });
